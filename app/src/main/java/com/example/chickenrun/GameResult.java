...
import com.example.chickenrun.gameRoom.retireUserList;
        ...
        OnMapReadyCallback, GoogleMap.OnMapClickListener, GoogleMap.OnMapLongClickListener
{
    ...
    // 1위 유저가 관리할 리타이어 리스트
    private List<retireUserList> List_retierUser;
    retireUserList retierUserList;
...
/*         if (GET_IS_HOST)
    ...
    private void getSocketMessage(String message)
    {
        ...
        } else
        {
            ...
            // 1위 다음으로 골인한 유저들은 1위 유저가 정리한 순위표를 받는다.
            ...
            // 1. 리타이어 한 유저들은 자신의 이동 거리를 1등 유저에게 전달한다.
            // 2. 1위 유저는 리타이어 유저들의 신호를 받고 거리순 정렬을 적용하여 순위표를 정리한다.
            // 3. 아래 조건문에서 최종 정리된 순위표를 전달 받는다. (GameProgressRetireSignal)
            // (1위가 아래 조건문에서 리타이어 유저를 정리하기)
            if (ioMessage[2].equals("GameProgressAddRetireSignal"))
            {                 // 1위 유저 이외의 유저는 LIST_LANK 변수를 사용하지 않기 때문에 null 체크 필요함
                if (LIST_LANK == null)
                {
                    Log.e(TAG, "getSocketMessage: 리타이어 유저의 순위를표 정리합니다. 잠시만 기다려 주세요");
                }

                // 1위 한 유저에게 리타이어 순위표 관리 권한 전담하기
                else
                {
                    if (LIST_LANK.get(0).getUserName().equals(GET_MY_NAME))
                    {
